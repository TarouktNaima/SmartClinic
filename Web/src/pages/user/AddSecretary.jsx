...
import { UserPlus, UserRound, Mail, Lock, ArrowLeft, ClipboardList } from "lucide-react";
      ...
      <main className="flex-1 p-5 lg:p-8">
        ...
        >
          <div className="w-16 h-16 rounded-2xl bg-emerald-500/20 flex items-center justify-center mb-6">
            <ClipboardList className="text-emerald-300" size={32} />
          </div>
          ...
          <p className="text-slate-400 mb-8">
            Créer un compte secrétaire pour gérer les statistiques.
          </p>

          <form onSubmit={handleSubmit} className="space-y-5">
            <InputBox icon={UserRound} placeholder="Nom secrétaire" value={name} setValue={setName} />
            <InputBox icon={Mail} type="email" placeholder="Email secrétaire" value={email} setValue={setEmail} />
            <InputBox icon={Lock} type="password" placeholder="Mot de passe" value={password} setValue={setPassword} />

            <button className="w-full flex items-center justify-center gap-2 rounded-2xl bg-emerald-600 hover:bg-emerald-500 text-white py-4 font-bold">
              <UserPlus size={20} />
              Ajouter Secrétaire
            </button>
          </form>
